...
def connect_to_datalake():
    ...
    jdbc_url = "jdbc:infordatalake://YOUR_TENANT"
...
def fetch_table_schema(conn):
    """
    Fetches the distinct table schemas from the database.

    Args:
        conn: Database connection object.

    Returns:
        obj_schema: DataFrame containing the distinct table schemas.
    """
...
def fetch_tables(selected_schema):
    """
    Fetches the tables for the selected schema.

    Args:
        selected_schema: Name of the schema.

    Returns:
        obj_table: DataFrame containing the table names.
    """
...
def fetch_fields(selected_table):
    """
    Fetches the fields (column names) for the selected table.

    Args:
        selected_table: Name of the table.

    Returns:
        obj_fields: DataFrame containing the column names.
    """
    conn = connect_to_datalake()
    cursor = conn.cursor()
...
with tabs_main[3]:
    ...
    if prompts:
        if st.button("respond"):
            #print(dfe)
            lake = SmartDatalake([uploaded_file])
